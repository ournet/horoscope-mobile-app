    ...
    render() {
        ...
        if (!items || !items.length) {
            return (
                <View style={styles.no_data}>
                    <Text style={styles.no_data_text}>{Locales.lang(lang).no_data_error()}</Text>
                </View>
            );
        }
        ...
        return (
            <View style={styles.container}>
                {items.map(item => <ReportItem truncate={true} noNumbers={true} noStats={true} key={item.id} report={item} lang={lang} />)}
            </View>
        );
    }
}
